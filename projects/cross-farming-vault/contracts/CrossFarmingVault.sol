...
contract CrossFarmingVault is Ownable, Pausable, ReentrancyGuard {
     ...
     * @param _nonce withdraw tx nonce from BSC chain.
     */
    function ackEmergencyWithdraw(address _user, uint256 _mcv2pid, uint64 _nonce) external nonReentrant onlySender {
        uint256 pid = poolMapping[_mcv2pid];
        require(!usedNonce[_user][pid][_nonce], "used nonce");
    ...
    function _fallback(address _user, uint256 _pid, uint256 _amount, uint64 _nonce) internal {
        PoolInfo storage pool = poolInfo[_pid];
