...
contract XYzKVault is Ownable, Pausable {
    ...
    /**
     * @notice Deposits funds into the XYzK Vault
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.xYzKAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the XYzK Vault
     */
     ...
     */
    function calculateHarvestXYzKRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingXYzK(0, address(this));
     ...
     * @return Returns total pending xYzK rewards
     */
    function calculateTotalPendingXYzKRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingXYzK(0, address(this));
        amount = amount.add(available());
    ...
    /**
     * @notice Withdraws from funds from the XYzK Vault
